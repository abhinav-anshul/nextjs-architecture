    ...
    {[...Array(12)].map((_, index) => (
      <Paper variant="outlined" key={index} sx={{ p: 1}}>
        <Stack spacing={2}>
          ...
          {index === 0 && (
            <Typography variant="caption" sx={{ paddingTop: 2, borderRadius: 1.5 }} >test</Typography>
          )}

          {index !== 2 && (
            <Typography variant="caption" sx={{ paddingTop: 2, borderRadius: 1.5 }} >hello world</Typography>
          )}
        </Stack>
      </Paper>
    ))}
